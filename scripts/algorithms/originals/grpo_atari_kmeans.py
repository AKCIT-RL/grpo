...
class Args:
    ...
    track: bool = False
    """if toggled, this experiment will be tracked with Weights and Bandages"""
...
if __name__ == "__main__":
    ...
    if args.track:
        ...
        wandb.init(
            ...
            monitor_gym=True,
            save_code=True,
            group=f"kmeans_atari_{args.env_id}__{args.exp_name}", 
        )
    ...
    # tensors with dtype=torch.float32 so it can work with mps
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for env_idx in range(args.num_envs):
            ...
            for t in reversed(range(args.num_steps)):
                current_return_sum = rewards[t, env_idx] + args.gamma * current_return_sum * (1.0 - dones[t, env_idx].float()) # Converter done para float
                returns_per_step[t, env_idx] = current_return_sum

        scaler = StandardScaler()
        with torch.no_grad():
            initial_obs_tensor = torch.tensor(initial_observations, device=device, dtype=torch.float32) / 255.0
            initial_obs_features = agent.network(initial_obs_tensor).cpu().numpy() 
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    logits_ref = reference_agent.actor(reference_agent.network(b_obs[mb_inds] / 255.0))
