...
class Args:
    ...
    kl_beta_coef: float = 0.04 # Coeficiente Beta para o termo de regularização KL na perda do GRPO
...
if __name__ == "__main__":
    ...
    logprobs_ref = torch.zeros((args.num_steps, args.num_envs), dtype=torch.float32).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Atualiza a política de referência (pi_ref) para ser uma cópia da política atual (pi_theta)
        # no início desta iteração. Isso serve como a "política antiga" para o cálculo da KL.
        reference_agent.load_state_dict(agent.state_dict())
        reference_agent.eval()
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                # Obtém logprobs da política de referência
        ...
        # Cálculo dos retornos descontados para cada trajetória (ambiente)
        returns_per_env = torch.zeros_like(rewards, dtype=torch.float32).to(device)
        ...
        advantages_grpo = (returns_per_env - mean_returns_batch) / std_returns_batch
        b_advantages_grpo = advantages_grpo.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Termo de regularização KL do GRPO
